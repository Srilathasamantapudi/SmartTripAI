...
(function () {
  ...
  var savedGuidesClose      = $("savedGuidesClose");
  var savedGuidesModalList  = $("savedGuidesModalList");
  ...
  function getSavedGuides() {
    try {
      var raw = localStorage.getItem(SAVED_GUIDES_KEY);
  ...
  function saveGuideToStorage(guideData) {
    var guides = getSavedGuides();
    var foundIndex = -1;
    for (var i = 0; i < guides.length; i++) {
      if (guides[i].query === guideData.query &&
          guides[i].result &&
          guideData.result &&
          guides[i].result.answer === guideData.result.answer) {
        foundIndex = i;
        break;
      }
    }
    if (foundIndex >= 0) {
      guides[foundIndex].savedAt = guideData.savedAt;
    } else {
      guides.unshift(guideData);
    }
    try {
  ...
  function deleteSavedGuide(id) {
    var guides = getSavedGuides().filter(function (g) { return g.id !== id; });
    try {
      localStorage.setItem(SAVED_GUIDES_KEY, JSON.stringify(guides));
    } catch (e) {}
    renderSavedGuides();
  ...
  function updateSavedGuidesBadge() {
    var count = getSavedGuides().length;
    var topbarBtn = $("savedGuidesTopbarBtn");
    if (topbarBtn) {
      topbarBtn.title = count ? "Saved Guides (" + count + ")" : "Saved Guides";
    }
  }

  function renderSavedGuides() {
    if (!savedGuidesModalList) return;
    savedGuidesModalList.innerHTML = "";

    var guides = getSavedGuides();
    if (guides.length === 0) {
      var empty = el("div", "saved-guides-empty");
      empty.appendChild(el("div", "saved-empty-icon", "💾"));
      empty.appendChild(el("strong", null, "No saved guides yet"));
      empty.appendChild(el("p", null, "Click \"💾 Save Guide\" on any generated report to store it here for easy access."));
      savedGuidesModalList.appendChild(empty);
      return;
    }

    guides.forEach(function (guide) {
      var item = el("div", "saved-guide-item");

      var itemHead = el("div", "saved-guide-head");
      var titleBox = el("div", "saved-guide-title-box");
      titleBox.appendChild(el("strong", "saved-guide-title", guide.query || "Travel Guide"));
      titleBox.appendChild(el("small", "saved-guide-meta", "Saved " + (guide.savedAt || "recently")));
      itemHead.appendChild(titleBox);

      var actions = el("div", "saved-guide-actions");

      var openBtn = el("button", "btn btn-primary btn-sm", "Open Guide");
      openBtn.type = "button";
      openBtn.addEventListener("click", function () {
        openSavedGuide(guide);
      });
      actions.appendChild(openBtn);

      var delBtn = el("button", "icon-btn sm");
      delBtn.type = "button";
      delBtn.title = "Delete saved guide";
      delBtn.setAttribute("aria-label", "Delete saved guide");
      delBtn.appendChild(icon("M3 6h18m-2 0v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6m3 0V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"));
      delBtn.addEventListener("click", function (e) {
        e.stopPropagation();
        deleteSavedGuide(guide.id);
      });
      ...
      var snippetText = "";
      if (guide.result && guide.result.answer) {
        snippetText = toText(guide.result.answer).replace(/[#*`_]/g, "").slice(0, 160).trim();
        if (snippetText) snippetText += "…";
      }
      if (snippetText) {
        item.appendChild(el("p", "saved-guide-snippet", snippetText));
      }

      savedGuidesModalList.appendChild(item);
    });
  }

  function openSavedGuide(guide) {
    closeSavedGuidesModal();
    closeSettings();
    messages.innerHTML = "";

    if (guide.query) {
      renderUser(guide.query);
    }

    var res = guide.result || { answer: guide.answer, __query: guide.query };
    res.__query = guide.query || res.__query;
  ...
  function renderUser(text) {
    ...
    var editBtn = el("button", "btn-prompt-edit", "✏️ Edit");
    editBtn.type = "button";
    editBtn.title = "Edit prompt";
    wrap.appendChild(editBtn);

    var editBox = el("div", "prompt-edit-box");
    editBox.hidden = true;

    var textarea = el("textarea", "prompt-edit-textarea");
    textarea.rows = 2;
    textarea.value = currentText;
    editBox.appendChild(textarea);

    var actionRow = el("div", "prompt-edit-actions");

    var cancelBtn = el("button", "btn btn-ghost btn-sm", "Cancel");
  ...
  function renderResult(result) {
    ...
    saveGuideBtn.innerHTML = '<span aria-hidden="true">💾</span> <span class="btn-label">Save Guide</span>';
    saveGuideBtn.addEventListener("click", function () {
      var guideData = {
        id: "guide_" + Date.now(),
        query: result.__query || "Custom Travel Guide",
      ...
      saveGuideToStorage(guideData);
      toast("Saved successfully");
      var lbl = saveGuideBtn.querySelector(".btn-label");
      if (lbl) {
        lbl.textContent = "Saved ✓";
        setTimeout(function () { lbl.textContent = "Save Guide"; }, 2000);
      }
    });
  ...
  function renderError(message, kind) {
    ...
    if (kind === "setup") {
      ...
      resetBtn.title = "Clear browser session keys and use the working server .env";
